...
const SingleUserDetails = (props) => {
      ...
      const fetchUser = useCallback(async() => {
        try{
          ...
          const response = await axios({
              method: apiSpec.USER_DETAILS.operation,
              url: apiSpec.USER_DETAILS.url,
              headers:{
    ...
    const handleAccoutBlock = useCallback(async(action) => {
        try{
          setIsLoading(true);
          const response = await axios({
              method: apiSpec.BLOCK_USER.operation,
              url: apiSpec.BLOCK_USER.url,
              headers:{
                  'Content-Type': 'application/json',
                  'Authorization':`Bearer ${jwt}`
              },
    ...
    return (
        ...
        >
            {({errors}) => (
                <Card style={{height:'100%', width:'100%', marginTop:'0', borderRadius:'25px', padding:'1%'}}>
                <Form>
                <Row>
                <Col md={6}>
                ...
                <Row style={{marginTop:'5%'}}>
                <h3>Zablokuj/usuń konto</h3>
                <Col md={6}>
                    <div className="form-group">
                        {
                            user.blocked?
                            <Button type="submit" class="btn btn-warning" onClick = {async() => {
                                await handleAccoutBlock('block');
                                setBlockAction(!blockAction);
                            }
                            } style={{marginRight:'16%'}}>Odblokuj konto</Button>
                            :
                            <Button type="submit" class="btn btn-warning" onClick = {async() => {
                                await handleAccoutBlock('block')
                                setBlockAction(!blockAction);
                            }
                             } style={{marginRight:'16%'}}>Zablokuj konto</Button>
                        }
                    </div>
                </Col>
                <Col md={6}>
                    <div className="form-group">
                    <Button type="submit" class="btn btn-danger" onClick = {async() => {
                        await handleAccoutBlock('delete');
                        history.push("account/adm/users");
                    }
                } style={{marginRight:'16%'}}>Usuń konto</Button>
                    </div>
                    </Col>
                </Row>
                </Form>
