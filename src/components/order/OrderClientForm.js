...
const OrderClientForm = (props) => {
  ...
  const addOrderApi = async function (user) {
    try {
      ...
      const accessories = orderedItems.filter(
        (item) => item.itemType === "ACCESSORIES"
      );
      const workshop = orderedItems.filter(
        (item) => item.itemType === "WORKSHOP"
      );
      const parts = orderedItems.filter((item) => item.itemType === "PARTS");
      const bags = accessories.filter(
        (item) => item.accessoriesType === "BAGS"
      );
      const bottles = accessories.filter(
        (item) => item.accessoriesType === "BOTTLE"
      );
      const fenders = accessories.filter(
        (item) => item.accessoriesType === "FENDERS"
      );
      const pumps = accessories.filter(
        (item) => item.accessoriesType === "PUMP"
      );
      ...
      const maintenances = workshop.filter(
        (item) => item.workshopType === "MAINTENANCE"
      );
  ...
  const fetchApi = useCallback(async () => {
    const response = await axios.get(apiSpec.USER_DETAILS.url, {
      params: { username },
    });
    const data = response.data;
    setUser(data);
      ...
      <div>
        ...
        <Formik
          ...
          validate={(values) => {
            ...
            if (statute === false) {
              errors.statute =
                "Zamówienie dostępne po zaakceptowaniu regulaminu";
            }
            if (!accountAddress) {
              errors = {};
            }
        ...
        >
          {({ isSubmitting, setFieldValue }) => (
            <Form>
              {!accountAddress ? (
                <div>
                  ...
                  <Field
                    type="address"
                    name="localNumber"
                    placeholder={user.address?.localNumber}
                    style={{ width: "15%" }}
                    disabled={true}
                  />
                  <br />
                  ...
                  <Field
                    type="phone"
                    name="phone"
                    placeholder={user.address?.phone}
                    className="form-control"
                    disabled={true}
                  />
                  <Field
                    type="email"
                    name="email"
                    placeholder={user.address?.email}
                    className="form-control"
                    disabled={true}
                  />
                </div>
              ...
              >
                Chcę wprowadzić inny adres dostawy
              </Button>
              <hr />
              <Row>
                <ShipmentDetails
